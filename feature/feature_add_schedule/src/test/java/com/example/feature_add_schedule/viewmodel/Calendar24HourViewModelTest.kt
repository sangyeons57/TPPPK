package com.example.feature_add_schedule.viewmodel
...
class Calendar24HourViewModelTest {
    ...
    private lateinit var viewModel: com.example.feature_calendar_24hour.Calendar24HourViewModel
    ...
    fun `초기화 시 선택된 날짜의 일정을 성공적으로 로드해야 함`() = coroutinesTestRule.runBlockingTest {
        ...
        viewModel = com.example.feature_calendar_24hour.Calendar24HourViewModel(
            savedStateHandle,
        ...
        assertTrue(uiState is com.example.feature_calendar_24hour.Calendar24HourUiState.Success)

        val successState =
            uiState as com.example.feature_calendar_24hour.Calendar24HourUiState.Success
    ...
    fun `일정 로딩 중 오류 발생 시 에러 상태로 업데이트되어야 함`() = coroutinesTestRule.runBlockingTest {
        ...
        // Then: 에러 상태 확인
        val uiState = viewModel.uiState.getValue()
        assertTrue(uiState is com.example.feature_calendar_24hour.Calendar24HourUiState.Error)

        val errorState = uiState as com.example.feature_calendar_24hour.Calendar24HourUiState.Error
    ...
    fun `일정 삭제 성공 시 UI 상태에서 제거되고 성공 스낵바가 표시되어야 함`() = coroutinesTestRule.runBlockingTest {
        ...
        // 이벤트 수집기 설정
        val eventCollector =
            EventCollector<com.example.feature_calendar_24hour.Calendar24HourEvent>()
        eventCollector.collectFrom(coroutinesTestRule.testCoroutineScope, viewModel.eventFlow)
        
        // 초기 상태 확인
        val initialState =
            viewModel.uiState.getValue() as com.example.feature_calendar_24hour.Calendar24HourUiState.Success
        ...
        assertTrue(event is com.example.feature_calendar_24hour.Calendar24HourEvent.ShowSnackbar)
        assertTrue(
            (event as com.example.feature_calendar_24hour.Calendar24HourEvent.ShowSnackbar).message.contains(
                "삭제되었습니다"
            )
        )
    }
    ...
    fun `뒤로 가기 버튼 클릭 시 NavigateBack 이벤트가 발생해야 함`() = coroutinesTestRule.runBlockingTest {
        ...
        assertTrue(eventCollector.events.isNotEmpty())
        val event = eventCollector.events.first()
        assertTrue(event is com.example.feature_calendar_24hour.Calendar24HourEvent.NavigateBack)
    }
    ...
    fun `일정 추가 버튼 클릭 시 NavigateToAddSchedule 이벤트가 발생해야 함`() = coroutinesTestRule.runBlockingTest {
        ...
        assertTrue(eventCollector.events.isNotEmpty())
        val event = eventCollector.events.first()
        assertTrue(event is com.example.feature_calendar_24hour.Calendar24HourEvent.NavigateToAddSchedule)
    }
    ...
    fun `일정 클릭 시 NavigateToScheduleDetail 이벤트가 발생해야 함`() = coroutinesTestRule.runBlockingTest {
        ...
        assertTrue(eventCollector.events.isNotEmpty())
        val event = eventCollector.events.first()
        assertTrue(event is com.example.feature_calendar_24hour.Calendar24HourEvent.NavigateToScheduleDetail)
        assertEquals(
            scheduleId,
            (event as com.example.feature_calendar_24hour.Calendar24HourEvent.NavigateToScheduleDetail).scheduleId
        )
    }
    ...
    fun `일정 롱클릭 시 ShowScheduleEditDialog 이벤트가 발생해야 함`() = coroutinesTestRule.runBlockingTest {
        ...
        assertTrue(eventCollector.events.isNotEmpty())
        val event = eventCollector.events.first()
        assertTrue(event is com.example.feature_calendar_24hour.Calendar24HourEvent.ShowScheduleEditDialog)
        assertEquals(
            scheduleId,
            (event as com.example.feature_calendar_24hour.Calendar24HourEvent.ShowScheduleEditDialog).scheduleId
        )
    }
