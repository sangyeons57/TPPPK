...
class JoinProjectViewModelTest {
    ...
    fun setup() {
        ...
        // ViewModel 초기화
        viewModel = JoinProjectViewModel(savedStateHandle, fakeProjectRepository)
    }
    ...
    fun `유효한 초대 코드로 참여 시 성공 이벤트가 발생해야 함`() = coroutinesTestRule.runBlockingTest {
        ...
        // 이벤트 수집기 설정
        val eventCollector = EventCollector<JoinProjectEvent>()
        ...
        assertTrue(eventCollector.events[0] is JoinProjectEvent.ClearFocus)
        assertTrue(eventCollector.events[1] is JoinProjectEvent.ShowSnackbar)
        assertTrue(eventCollector.events[2] is JoinProjectEvent.JoinSuccess)
        
        // JoinSuccess 이벤트의 projectId가 올바른지 확인
        val successEvent = eventCollector.events[2] as JoinProjectEvent.JoinSuccess
    ...
    fun `로딩 중에 추가 참여 요청을 방지해야 함`() = coroutinesTestRule.runBlockingTest {
        ...
        val mutableStateFlow = privateUiState.get(viewModel) as MutableStateFlow<JoinProjectUiState>
