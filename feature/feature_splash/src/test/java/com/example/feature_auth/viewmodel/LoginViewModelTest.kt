...
class LoginViewModelTest {
    ...
    private lateinit var viewModel: com.example.feature_login.LoginViewModel
    ...
    fun setup() {
        ...
        viewModel = com.example.feature_login.LoginViewModel(fakeAuthRepository)
    }
    ...
    fun `올바른 이메일과 비밀번호로 로그인 시 성공 이벤트가 발생해야 함`() = coroutinesTestRule.runBlockingTest {
        ...
        val eventCollector = EventCollector<com.example.feature_login.LoginEvent>()
        ...
        assertTrue(event is com.example.feature_login.LoginEvent.LoginSuccess)
        assertEquals(testUser.userId, (event as com.example.feature_login.LoginEvent.LoginSuccess).userId)
    }
    ...
    fun `로그인 중 오류 발생 시 스낵바 메시지가 표시되어야 함`() = coroutinesTestRule.runBlockingTest {
        ...
        assertTrue(eventCollector.events.isNotEmpty())
        val event = eventCollector.events.first()
        assertTrue(event is com.example.feature_login.LoginEvent.ShowSnackbar)
        assertTrue((event as com.example.feature_login.LoginEvent.ShowSnackbar).message.isNotEmpty())
    }
    ...
    fun `잘못된 비밀번호로 로그인 시 오류 메시지가 표시되어야 함`() = coroutinesTestRule.runBlockingTest {
        ...
        val event = eventCollector.events.first()
        assertTrue(event is com.example.feature_login.LoginEvent.ShowSnackbar)
        assertEquals("비밀번호가 일치하지 않습니다", (event as com.example.feature_login.LoginEvent.ShowSnackbar).message)
    }
    ...
    fun `비밀번호 찾기 버튼 클릭 시 NavigateToFindPassword 이벤트가 발생해야 함`() = coroutinesTestRule.runBlockingTest {
        ...
        assertTrue(eventCollector.events.isNotEmpty())
        val event = eventCollector.events.first()
        assertTrue(event is com.example.feature_login.LoginEvent.NavigateToFindPassword)
    }
    ...
    fun `회원가입 버튼 클릭 시 NavigateToSignUp 이벤트가 발생해야 함`() = coroutinesTestRule.runBlockingTest {
        ...
        assertTrue(eventCollector.events.isNotEmpty())
        val event = eventCollector.events.first()
        assertTrue(event is com.example.feature_login.LoginEvent.NavigateToSignUp)
    }
    ...
    fun `이메일 유효성 검사 실패 시 이메일 필드로 포커스 이동 요청이 발생해야 함`() = coroutinesTestRule.runBlockingTest {
        ...
        assertTrue(eventCollector.events.isNotEmpty())
        val event = eventCollector.events.first()
        assertTrue(event is com.example.feature_login.LoginEvent.RequestFocus)
        assertEquals(LoginFormFocusTarget.EMAIL, (event as com.example.feature_login.LoginEvent.RequestFocus).target)
    }
    ...
    fun `비밀번호 유효성 검사 실패 시 비밀번호 필드로 포커스 이동 요청이 발생해야 함`() = coroutinesTestRule.runBlockingTest {
        ...
        val event = eventCollector.events.first()
        assertTrue(event is com.example.feature_login.LoginEvent.RequestFocus)
        assertEquals(LoginFormFocusTarget.PASSWORD, (event as com.example.feature_login.LoginEvent.RequestFocus).target)
    }
} 