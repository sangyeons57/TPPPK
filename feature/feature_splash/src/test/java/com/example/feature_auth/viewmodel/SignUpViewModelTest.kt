...
class SignUpViewModelTest {
    ...
    private lateinit var viewModel: com.example.feature_signup.SignUpViewModel
    ...
    fun setup() {
        ...
        viewModel = com.example.feature_signup.SignUpViewModel(fakeAuthRepository)
    }
    ...
    fun `유효한 정보로 회원가입 시 성공 이벤트가 발생해야 함`() = coroutinesTestRule.runBlockingTest {
        ...
        val eventCollector = EventCollector<com.example.feature_signup.SignUpEvent>()
        ...
        val snackbarEvent = eventCollector.events.find { it is com.example.feature_signup.SignUpEvent.ShowSnackbar }
        val navigateEvent = eventCollector.events.find { it is com.example.feature_signup.SignUpEvent.NavigateToLogin }
        
        assertNotNull(snackbarEvent)
        assertNotNull(navigateEvent)
        assertEquals("회원가입 성공! 로그인해주세요.", (snackbarEvent as com.example.feature_signup.SignUpEvent.ShowSnackbar).message)
        assertTrue(navigateEvent is com.example.feature_signup.SignUpEvent.NavigateToLogin)
    ...
    fun `회원가입 중 오류 발생 시 스낵바 메시지가 표시되어야 함`() = coroutinesTestRule.runBlockingTest {
        ...
        assertTrue(event is com.example.feature_signup.SignUpEvent.ShowSnackbar)
        assertTrue((event as com.example.feature_signup.SignUpEvent.ShowSnackbar).message.isNotEmpty())
    ...
    fun `이미 사용 중인 이메일로 회원가입 시 오류 메시지가 표시되어야 함`() = coroutinesTestRule.runBlockingTest {
        ...
        val event = eventCollector.events.first()
        assertTrue(event is com.example.feature_signup.SignUpEvent.ShowSnackbar)
        assertEquals("이미 사용 중인 이메일입니다", (event as com.example.feature_signup.SignUpEvent.ShowSnackbar).message)
    }
    ...
    fun `잘못된 이메일 형식으로 회원가입 시 포커스 요청이 발생해야 함`() = coroutinesTestRule.runBlockingTest {
        ...
        assertTrue(eventCollector.events.isNotEmpty())
        val event = eventCollector.events.first()
        assertTrue(event is com.example.feature_signup.SignUpEvent.RequestFocus)
        assertEquals(SignUpFormFocusTarget.EMAIL, (event as com.example.feature_signup.SignUpEvent.RequestFocus).target)
