...
export class FirestoreImageDataSource implements ImageRepository {
  ...
  async save(image: ProcessedImageEntity): Promise<CustomResult<ProcessedImageEntity>> {
    try {
      ...
      return Result.failure(new InternalError(`Failed to save image: ${error instanceof Error ? error.message : 'Unknown error'}`));
    }
  }
  ...
  async update(image: ProcessedImageEntity): Promise<CustomResult<ProcessedImageEntity>> {
    try {
      ...
      await this.collection.doc(image.id).update(data);
      return Result.success(image);
    } catch (error) {
      return Result.failure(new InternalError(`Failed to update image: ${error instanceof Error ? error.message : 'Unknown error'}`));
    }
  }
  ...
  async deleteByUrl(url: ImageUrl): Promise<CustomResult<void>> {
    try {
      ...
      return Result.failure(new InternalError(`Failed to delete image by URL: ${error instanceof Error ? error.message : 'Unknown error'}`));
    }
  }
...
export class FirebaseStorageService implements ImageStorageService {
  ...
  async generateThumbnail(imageBuffer: Buffer, width: number, height: number): Promise<CustomResult<Buffer>> {
    try {
      ...
      return Result.failure(new InternalError(`Failed to generate thumbnail: ${error instanceof Error ? error.message : 'Unknown error'}`));
    }
  }
  ...
  async validateImageFormat(contentType: string): Promise<CustomResult<boolean>> {
    try {
      ...
      const isValid = IMAGE_PROCESSING.SUPPORTED_FORMATS.includes(format);
      return Result.success(isValid);
    } catch (error) {
      return Result.failure(new InternalError(`Failed to validate image format: ${error instanceof Error ? error.message : 'Unknown error'}`));
    }
  }
