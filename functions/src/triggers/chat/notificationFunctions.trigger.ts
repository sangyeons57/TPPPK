...
// =====================================
// 1. Message Mention Notification (Firestore Trigger)
// =====================================
export const onMessageMentionNotification = firestoreFunctions.onDocumentCreated({
...
// =====================================
// 2. General Message Notification (Firestore Trigger)
// =====================================
export const onNewMessageNotification = firestoreFunctions.onDocumentCreated({
  document: "messages/{messageId}",
  region: RUNTIME_CONFIG.REGION,
  memory: RUNTIME_CONFIG.MEMORY,
  timeoutSeconds: RUNTIME_CONFIG.TIMEOUT_SECONDS,
...
// =====================================
// 3. Custom FCM Notification (HTTP Callable)
// =====================================
export const sendCustomNotification = httpsFunctions.onCall({
  region: RUNTIME_CONFIG.REGION,
  memory: RUNTIME_CONFIG.MEMORY,
  timeoutSeconds: RUNTIME_CONFIG.TIMEOUT_SECONDS,
}, async (request) => {
  ...
  if (!request.auth) {
    throw new httpsFunctions.HttpsError(
      "unauthenticated",
...
// =====================================
// 4. Update FCM Token (HTTP Callable)
// =====================================
export const updateFcmToken = httpsFunctions.onCall({
  region: RUNTIME_CONFIG.REGION,
  memory: RUNTIME_CONFIG.MEMORY,
  timeoutSeconds: RUNTIME_CONFIG.TIMEOUT_SECONDS,
}, async (request) => {
...
async function processMention(
  ...
  try {
    if (mentionType === "USER") {
      await processUserMention(mentionId, messageData, messageId, displayName);
...
/**
 * Process DM message notification
 */
...
/**
 * Process project channel message notification
 */
