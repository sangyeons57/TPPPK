...
class TaskContainerRemoteDataSourceImpl @Inject constructor(
...
) : DefaultDatasourceImpl<TaskContainerUnifiedDTO>(firestore, TaskContainerUnifiedDTO::class.java), TaskContainerRemoteDataSource {
    
    /**
     * 고정된 container ID로 단일 TaskContainer 문서를 관찰합니다.
     */
    fun observeContainer(): Flow<TaskContainerUnifiedDTO?> {
        return observeById(TaskContainerUnifiedDTO.FIXED_CONTAINER_ID)
    }
    ...
    override fun observeAll(): Flow<List<TaskContainerUnifiedDTO>> {
        return super.observeAll().map { documents ->
            documents.filter { it.type == TaskContainerUnifiedDTO.TYPE_CONTAINER }
        }
    }
     ...
     */
    suspend fun createContainer(dto: TaskContainerUnifiedDTO): CustomResult<DocumentId, Exception> {
        val containerDto = dto.copy(
            id = TaskContainerUnifiedDTO.FIXED_CONTAINER_ID,
    ...
    suspend fun updateContainer(fields: Map<String, Any?>): CustomResult<DocumentId, Exception> {
        return update(TaskContainerUnifiedDTO.FIXED_CONTAINER_ID, fields)
    }
    ...
    suspend fun deleteContainer(): CustomResult<DocumentId, Exception> {
        return delete(TaskContainerUnifiedDTO.FIXED_CONTAINER_ID)
    }
}