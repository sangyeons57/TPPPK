...
import com.example.data.model.mapper.toDomain // DTO -> Domain 확장 함수 import
import com.example.data.model.mapper.toDto // Domain -> DTO 확장 함수 import
import com.example.domain.model.Schedule
import com.example.domain.repository.ScheduleRepository
import com.google.firebase.Timestamp
import java.time.* // LocalDate, YearMonth, ZoneId 등 사용
...
class ScheduleRepositoryImpl @Inject constructor(
    ...
    override suspend fun getSchedulesForDate(date: LocalDate): Result<List<Schedule>> {
        return runCatching {
            ...
            val startOfDay = Timestamp(startInstant)
            val endOfDay = Timestamp(endInstant)

            // 데이터 소스 호출 및 DTO 리스트를 도메인 모델 리스트로 변환
            remoteDataSource.getSchedulesForDate(startOfDay, endOfDay).toDomain()
        }
    ...
    override suspend fun getScheduleSummaryForMonth(yearMonth: YearMonth): Result<Set<LocalDate>> {
        return runCatching {
            ...
            val startOfMonth = Timestamp(startInstant)
            val endOfMonth = Timestamp(endInstant)
            ...
            schedulesDto.mapNotNull { it.startTime } // startTime이 null인 경우 제외
                .map { timestamp ->
    ...
    override suspend fun getScheduleDetail(scheduleId: String): Result<Schedule> {
        return runCatching {
            ...
            remoteDataSource.getScheduleDetail(scheduleId).toDomain()
        }
    }
    ...
    override suspend fun addSchedule(newScheduleData: Schedule): Result<Unit> {
        return runCatching {
            ...
            val scheduleDto = newScheduleData.toDto()
    ...
    override suspend fun updateSchedule(updatedScheduleData: Schedule): Result<Unit> {
        return runCatching {
            ...
            // 도메인 모델을 DTO로 변환 (id는 DTO 변환 시 제외됨)
            val scheduleDto = updatedScheduleData.toDto()
            // 데이터 소스 호출
