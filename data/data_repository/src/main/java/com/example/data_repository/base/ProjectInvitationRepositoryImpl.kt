...
class ProjectInvitationRepositoryImpl @Inject constructor(
    ...
    private fun mapToProjectInvitation(data: Map<String, Any?>): ProjectInvitation {
        return ProjectInvitation.fromDataSource(
            id = DocumentId(data[AggregateRoot.KEY_ID] as String),
            status = InviteStatus.fromString(data[ProjectInvitation.KEY_STATUS] as String?),
            inviterId = UserId(data[ProjectInvitation.KEY_INVITER_ID] as String),
            projectId = ProjectId(data[ProjectInvitation.KEY_PROJECT_ID] as String),
            expiresAt = (data[ProjectInvitation.KEY_EXPIRES_AT] as? Long)?.let {
                Instant.ofEpochMilli(
                    it
                )
            },
            createdAt = (data[AggregateRoot.KEY_CREATED_AT] as? Long)?.let { Instant.ofEpochMilli(it) },
            updatedAt = (data[AggregateRoot.KEY_UPDATED_AT] as? Long)?.let { Instant.ofEpochMilli(it) },
        )
    }
}