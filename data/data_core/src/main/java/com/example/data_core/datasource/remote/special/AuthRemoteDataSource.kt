...
class AuthRemoteDataSourceImpl @Inject constructor(
    ...
    override fun observeAuthState(): Flow<CustomResult<FirebaseUser, Exception>> = callbackFlow {
        ...
        awaitClose { 
            Log.d("AuthRemoteDataSource", "Auth state listener removed")
    ...
    override suspend fun signIn(
        ...
        withContext(Dispatchers.IO) {
            Log.d("AuthRemoteDataSource", "Attempting Firebase signIn for email: ${email.value}")
            resultTry {
                ...
                Log.d("AuthRemoteDataSource", "Firebase signIn successful, UID: $uid")
                uid
            }
        }

    override suspend fun signOut(): CustomResult<Unit, Exception> = withContext(Dispatchers.IO) {
        Log.d("AuthRemoteDataSource", "Attempting Firebase signOut")
        resultTry {
            auth.signOut()
    ...
    private inline fun <T> resultTry(block: () -> T): CustomResult<T, Exception> {
        return try {
            ...
            Log.e("AuthRemoteDataSource", "Exception in resultTry: ${e.message}", e)
