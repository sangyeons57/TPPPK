...
import com.example.data.datasource.local.LocalDmWrapperDataSource
...
class LocalDMWrapperRepositoryImpl @Inject constructor(
    ...
    override fun observeDMWrappersByUserName(userName: String, limit: Int): Flow<List<DMWrapper>> {
        ...
        return kotlinx.coroutines.flow.map(observeAllDMWrappers()) { wrappers ->
            wrappers.filter { wrapper ->
                wrapper.otherUserName.value.contains(userName, ignoreCase = true)
            }.take(limit)
        }
    }
    ...
    override fun observeDMWrapperUpdatedAt(wrapperId: String): Flow<Long?> {
        ...
        return kotlinx.coroutines.flow.map(observeDMWrapperById(wrapperId)) { wrapper ->
            wrapper?.updatedAt?.toEpochMilli()
        }
    }
    ...
    override fun observeDMWrappersWithRecentMessages(): Flow<List<DMWrapper>> {
        ...
        return kotlinx.coroutines.flow.map(observeAllDMWrappers()) { wrappers ->
            wrappers.filter { it.lastMessagePreview != null }
        }
    }
    ...
    override fun observeDMWrappers(wrapperIds: List<String>): Flow<List<DMWrapper>> {
        ...
        return kotlinx.coroutines.flow.map(observeAllDMWrappers()) { wrappers ->
            wrappers.filter { it.id.value in wrapperIds }
        }
    }
    ...
    override suspend fun searchDMWrappersByUserName(userName: String, limit: Int): List<DMWrapper> {
        ...
        return try {
            getAllDMWrappers().filter { wrapper ->
                wrapper.otherUserName.value.contains(userName, ignoreCase = true)
            }.take(limit)
        } catch (e: Exception) {
    ...
    override suspend fun getDMWrappersByIds(wrapperIds: List<String>): List<DMWrapper> {
        ...
        return try {
            wrapperIds.mapNotNull { wrapperId ->
                getDMWrapperById(wrapperId)
            }
        } catch (e: Exception) {
    ...
    override suspend fun getDMWrappersWithRecentMessages(): List<DMWrapper> {
        ...
        return try {
            getAllDMWrappers().filter { it.lastMessagePreview != null }
        } catch (e: Exception) {
    ...
    override suspend fun getDMWrappersByOtherUsers(otherUserIds: List<String>): List<DMWrapper> {
        ...
        return try {
            otherUserIds.mapNotNull { otherUserId ->
                getDMWrapperByOtherUser(otherUserId)
            }
        } catch (e: Exception) {
    ...
    override suspend fun updateDMWrapperOtherUser(
        ...
        return try {
            ...
            // 2. 다른 사용자 변경
            currentWrapper.changeOtherUser(newOtherUserId)

            // 3. 저장 (Outbox 포함)
            saveDMWrapper(currentWrapper)
    ...
    override suspend fun dmWrapperExistsWithOtherUser(otherUserId: String): Boolean {
        return try {
            getDMWrapperByOtherUser(otherUserId) != null
        } catch (e: Exception) {
    ...
    override suspend fun getDMWrapperCountWithRecentMessages(): Int {
        return try {
            getDMWrappersWithRecentMessages().size
        } catch (e: Exception) {
