...
class LocalMessageRepositoryImpl @Inject constructor(
    ...
    override fun observeUnreadMessageCount(
        ...
        // LocalDataSource에서 unread count를 Flow로 관찰하는 메서드가 필요
        // 현재는 단순 조회로 대체 (나중에 Flow 버전 추가 필요)
        return kotlinx.coroutines.flow.flow {
            while (true) {
                emit(
                    localMessagesDataSource.getUnreadMessageCount(
                        channelId,
                        userId,
                        lastReadTimestamp
                    )
                )
                kotlinx.coroutines.delay(1000) // 1초마다 업데이트 (임시)
            }
        }
    }
