...
import com.example.data.datasource.local.LocalDmChannelsDataSource
...
class LocalDMChannelRepositoryImpl @Inject constructor(
    ...
    override fun observeDMChannelsByStatus(status: DMChannelStatus): Flow<List<DMChannel>> {
        ...
        // Filter from all channels since specific status observation may not be available
        return kotlinx.coroutines.flow.map(observeAllDMChannels()) { channels ->
    ...
    override fun observeDMChannelsByActiveStatus(isActive: Boolean): Flow<List<DMChannel>> {
        ...
        // Filter from all channels
        return kotlinx.coroutines.flow.map(observeAllDMChannels()) { channels ->
            channels.filter { it.isActive() == isActive }
        }
    }
    ...
    override fun observeDMChannelUpdatedAt(channelId: String): Flow<Long?> {
        ...
        // Map from channel observation to updatedAt timestamp
        return kotlinx.coroutines.flow.map(observeDMChannelById(channelId)) { channel ->
    ...
    override fun observeBlockedDMChannelsByUser(userId: String): Flow<List<DMChannel>> {
        ...
        // Filter user's channels for blocked status
        return kotlinx.coroutines.flow.map(observeDMChannelsByUser(userId)) { channels ->
            channels.filter { it.isBlocked() }
        }
    }
    ...
    override suspend fun getDMChannelsByStatus(status: DMChannelStatus): List<DMChannel> {
        ...
        return try {
            getAllDMChannels().filter { it.status == status }
        } catch (e: Exception) {
    ...
    override suspend fun getBlockedDMChannelsByUser(userId: String): List<DMChannel> {
        ...
        return try {
            getDMChannelsByUser(userId).filter { it.isBlocked() }
        } catch (e: Exception) {
    ...
    override suspend fun dmChannelExistsBetweenUsers(user1Id: String, user2Id: String): Boolean {
        return try {
            getDMChannelBetweenUsers(user1Id, user2Id) != null
        } catch (e: Exception) {
    ...
    override suspend fun getDMChannelCountByStatus(status: DMChannelStatus): Int {
        return try {
            getDMChannelsByStatus(status).size
        } catch (e: Exception) {
