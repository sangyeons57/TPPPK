...
import com.example.data.datasource.local.LocalReactionsDataSource
...
class LocalReactionRepositoryImpl @Inject constructor(
    ...
    override fun observeReactionsByEmoji(emoji: Emoji): Flow<List<Reaction>> {
        ...
        return localReactionsDataSource.observeReactionsByEmoji(emoji)
    }
    ...
    override fun observeReactionsByMessageAndEmoji(
        ...
        return localReactionsDataSource.observeReactionsByMessageAndEmoji(messageId, emoji)
    }
    ...
    override fun observeReactionsByUserAndEmoji(
        ...
        return kotlinx.coroutines.flow.map(observeReactionsByUser(userId)) { reactions ->
    ...
    override fun observeReactionUpdatedAt(reactionId: String): Flow<Long?> {
        ...
        return kotlinx.coroutines.flow.map(observeReactionById(reactionId)) { reaction ->
            reaction?.updatedAt?.toEpochMilli()
        }
    }
    ...
    override fun observeReactionCountsByMessage(messageId: String): Flow<Map<Emoji, Int>> {
        ...
        return kotlinx.coroutines.flow.map(observeReactionsByMessage(messageId)) { reactions ->
            reactions.groupBy { it.emoji }.mapValues { it.value.size }
        }
    }

    override fun observeRecentReactions(timestamp: Instant): Flow<List<Reaction>> {
        Log.d(TAG, "observeRecentReactions: $timestamp")
        return kotlinx.coroutines.flow.map(observeAllReactions()) { reactions ->
            reactions.filter { it.createdAt.isAfter(timestamp) }
    ...
    override suspend fun getReactionsByEmoji(emoji: Emoji): List<Reaction> {
        ...
        return try {
            localReactionsDataSource.getReactionsByEmoji(emoji)
        } catch (e: Exception) {
    ...
    override suspend fun getReactionsByMessageAndEmoji(
        ...
        return try {
            localReactionsDataSource.getReactionsByMessageAndEmoji(messageId, emoji)
        } catch (e: Exception) {
    ...
    override suspend fun getReactionsByUserAndEmoji(userId: String, emoji: Emoji): List<Reaction> {
        ...
        return try {
            getReactionsByUser(userId).filter { it.emoji == emoji }
        } catch (e: Exception) {
    ...
    override suspend fun getReactionCountsByMessage(messageId: String): Map<Emoji, Int> {
        ...
        return try {
            getReactionsByMessage(messageId).groupBy { it.emoji }.mapValues { it.value.size }
        } catch (e: Exception) {
    ...
    override suspend fun getUniqueEmojisByMessage(messageId: String): List<Emoji> {
        ...
        return try {
            getReactionsByMessage(messageId).map { it.emoji }.distinct()
        } catch (e: Exception) {
    ...
    override suspend fun getRecentReactions(timestamp: Instant): List<Reaction> {
        ...
        return try {
            getAllReactions().filter { it.createdAt.isAfter(timestamp) }
                .sortedByDescending { it.createdAt }
        } catch (e: Exception) {
    ...
    override suspend fun getReactionCount(messageId: String, emoji: Emoji): Int {
        ...
        return try {
            getReactionsByMessageAndEmoji(messageId, emoji).size
        } catch (e: Exception) {
    ...
    override suspend fun deleteReactionsByMessageAndUser(
        ...
        return try {
            ...
            localReactionsDataSource.deleteReactionsByMessageAndUser(messageId, userId)
    ...
    override suspend fun deleteReactionsByMessageAndEmoji(
        ...
        return try {
            ...
            localReactionsDataSource.deleteReactionsByMessageAndEmoji(messageId, emoji)
    ...
    override suspend fun toggleReaction(
        ...
        return try {
            ...
            // 1. 기존 반응 확인
            val existingReactions = getReactionsByMessageAndUser(messageId, userId)
    ...
    override suspend fun reactionExists(messageId: String, userId: String, emoji: Emoji): Boolean {
        return try {
            getReactionsByMessageAndUser(messageId, userId).any { it.emoji == emoji }
        } catch (e: Exception) {
            Log.e(TAG, "reactionExists failed", e)
            false
        }
    ...
    override suspend fun getReactionCountByMessage(messageId: String): Int {
        return try {
            localReactionsDataSource.getReactionCountByMessage(messageId)
        } catch (e: Exception) {
    ...
    override suspend fun getReactionCountByEmoji(emoji: Emoji): Int {
        return try {
            localReactionsDataSource.getReactionCountByEmoji(emoji)
        } catch (e: Exception) {
    ...
    override suspend fun getUniqueEmojiCountByMessage(messageId: String): Int {
        return try {
            getUniqueEmojisByMessage(messageId).size
        } catch (e: Exception) {
