 ...
 * Firebase Function을 통해 사용자 ID를 기반으로 DM 채널을 생성합니다.
...
class AddDmChannelUseCase @Inject constructor(
    ...
    /**
     * 지정된 상대방 사용자 ID로 DM 채널을 생성하거나 가져옵니다.
     *
     * @param targetUserId DM 채널을 생성할 상대방의 사용자 ID.
     ...
     *         실패 사유는 사용자 ID가 비어있거나, 인증되지 않았거나, 대상 사용자를 찾을 수 없거나,
     *         자기 자신과의 DM을 시도했거나, DM 채널 생성 중 오류가 발생한 경우 등입니다.
     */
    operator fun invoke(targetUserId: String): Flow<CustomResult<DocumentId, Exception>> = flow {
        ...
        when (session) {
            is CustomResult.Success -> {
                ...
                val createDMResult = dmChannelRepository.createDMChannel(targetUserId)
                when (createDMResult) {
                    is CustomResult.Success -> {
                        ...
                        Log.d("AddDmChannelUseCase", "Response keys: ${resultData.keys}")
                        Log.d(
                            "AddDmChannelUseCase",
                            "Looking for key: ${FirebaseFunctionParameters.DM.CHANNEL_ID}"
                        )

                        // Firebase Function 응답 구조: {success: true, channelId: "...", ...}
                        // 최상위 레벨에서 channelId 직접 추출
                        val channelId =
                            resultData[FirebaseFunctionParameters.DM.CHANNEL_ID] as? String

                        // 대안 추출 시도 (하드코딩된 키로)
                        val alternativeChannelId = resultData["channelId"] as? String

                        Log.d("AddDmChannelUseCase", "Extracted channelId (constant): $channelId")
                        Log.d(
                            "AddDmChannelUseCase",
                            "Extracted channelId (hardcoded): $alternativeChannelId"
                        )

                        val finalChannelId = channelId ?: alternativeChannelId

                        if (finalChannelId != null) {
                            Log.d("AddDmChannelUseCase", "✅ Channel ID found: $finalChannelId")
                            emit(CustomResult.Success(DocumentId.from(finalChannelId)))
                        } else {
                            Log.e("AddDmChannelUseCase", "❌ Channel ID not found in response")
