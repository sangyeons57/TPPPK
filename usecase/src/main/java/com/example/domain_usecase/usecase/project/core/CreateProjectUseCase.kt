 ...
 * 2. 새 프로젝트를 생성합니다 ([com.example.domain_repository.base.ProjectRepository]).
 * 3. 생성된 프로젝트에 현재 사용자를 참여시킵니다 ([com.example.domain_repository.base.ProjectsWrapperRepository]).
 * 4. 새 프로젝트에 기본 "카테고리 없음" 카테고리를 자동으로 생성합니다 ([com.example.domain_repository.base.CategoryRepository]).
 *
    ...
    suspend operator fun invoke(name: String): CustomResult<DocumentId, Exception> {
        ...
        when (val session = authRepository.getCurrentUserSession()) {
            is CustomResult.Success -> {
                ...
                roleRepository.setCollection(CollectionPath.projectRoles(createdProjectId.value))
                Log.d("CreateProjectUseCase", "roleRepository collection set to project roles")
                ...
                memberRepository.setCollection(CollectionPath.projectMembers(createdProjectId.value))
                val owner : Member = Member.create(
                    id = DocumentId.from(session.data.userId),
                    roleIds = listOf(roleId)
                )
                Log.d("CreateProjectUseCase", "memberRepository collection set to project members")
                ...
                projectsWrapperRepository.setCollection(CollectionPath.userProjectWrappers(session.data.userId.value))
                ...
                categoryRepository.setCollection(CollectionPath.projectCategories(createdProjectId.value))
