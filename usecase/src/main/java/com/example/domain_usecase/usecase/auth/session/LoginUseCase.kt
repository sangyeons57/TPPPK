package com.example.domain_usecase.usecase.auth.session
...
class LoginUseCase @Inject constructor(
    ...
    suspend operator fun invoke(
        ...
        Log.d(TAG, "Attempting login for email=${email.value}")
        return when (val loginResult = authRepository.login(email, password)) {
            is CustomResult.Success -> {
                ...
                Log.d(TAG, "Fetching user details from server for userId=${userSession.userId}")
                when (val userResult = userRepository.findById(DocumentId.from(userSession.userId), Source.SERVER)) {
                    is CustomResult.Success -> {
                        ...
                        Log.d(TAG, "Fetched User from server: accountStatus=${user.accountStatus}")
                        if (user.accountStatus == UserAccountStatus.WITHDRAWN) {
                            Log.w(TAG, "Account is WITHDRAWN, logging out completely")
                            ...
                            Log.d(TAG, "Account is active, login successful")
                            loginResult
                        }
                    }
                    is CustomResult.Failure -> {
                        ...
                        Log.e(
                            TAG,
                            "Failed to fetch user details after login: ${userResult.error.message}",
                            userResult.error
                        )
                        authRepository.logoutCompletely()
                        ...
                        Log.e(TAG, "Unexpected userDetailsResult state: $userResult")
                        authRepository.logoutCompletely()
            ...
            is CustomResult.Failure -> {
                ...
                Log.e(TAG, "Authentication failed: ${loginResult.error.message}", loginResult.error)
                // Login authentication failed, return the original failure
                loginResult
            }
            else -> {
                Log.e(TAG, "Unexpected login result state: $loginResult")
