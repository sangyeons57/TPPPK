...
/**
 * ScopeMetadata 관련 Repository 인터페이스
 * SyncManager에서 동기화 메타데이터 관리를 위한 도메인 계층 인터페이스
 *
 * Clean Architecture 원칙에 따라 도메인 계층에서 데이터 계층의 추상화를 정의
 */
interface ScopeMetaDataRepository {
     ...
     */
    suspend fun findByKey(key: String): CustomResult<ScopeMetadata?, Exception>

    /**
     * 모든 ScopeMetadata 조회
     * @return 모든 ScopeMetadata 도메인 모델 목록
     */
    suspend fun findAll(): CustomResult<List<ScopeMetadata>, Exception>
     ...
     */
    suspend fun findByKeyPattern(keyPattern: String): CustomResult<List<ScopeMetadata>, Exception>
     ...
     */
    suspend fun findSyncedAfter(afterTimestamp: Instant): CustomResult<List<ScopeMetadata>, Exception>
     ...
     * @return 존재 여부
     */
    suspend fun existsByKey(key: String): CustomResult<Boolean, Exception>
     ...
     */
    suspend fun findErrorMetadata(minErrorCount: Long = 1): CustomResult<List<ScopeMetadata>, Exception>
     ...
     */
    suspend fun findActiveSyncMetadata(
        minSyncCount: Long,
     ...
     * @return 전체 메타데이터 개수
     */
    suspend fun count(): CustomResult<Int, Exception>

    /**
     * 오래된 메타데이터 정리 (특정 시간 이전 동기화된 것들)
     * @param beforeTimestamp 기준 시간 이전
     * @return 삭제된 행의 개수
     */
    suspend fun deleteOldMetadata(beforeTimestamp: Instant): CustomResult<Int, Exception>
     ...
     * @return 동기화 통계 정보
     */
    suspend fun getSyncStatistics(): CustomResult<SyncStatisticsData, Exception>
     ...
     */
    suspend fun findAllForDebug(): CustomResult<List<ScopeMetadata>, Exception>
}

/**
 * 동기화 통계 데이터 클래스 (도메인 모델)
 */
data class SyncStatisticsData(
    val totalCount: Int,
