...
/**
 * 태스크 데이터의 서버-로컬 간 충돌을 해결하는 UseCase
 * 충돌 감지 및 해결 전략을 적용합니다.
 */
class ResolveTaskDataConflictUseCase @Inject constructor(
    ...
    suspend operator fun invoke(
        ...
        return try {
            // 1. 로컬과 서버에서 데이터 조회
            val localResult = localRepository.getEntityById(taskId.value)
            val localTask = when (localResult) {
                is CustomResult.Success -> localResult.data
                is CustomResult.Failure -> null
                else -> null
            }
            
            remoteRepository.setCollection(CollectionPath.from("tasks"))
            val remoteResult = remoteRepository.findById(taskId)
            val remoteTask = when (remoteResult) {
            ...
            val conflictType = when {
                ...
                localTask == null -> ConflictType.LOCAL_MISSING
                remoteTask == null -> ConflictType.REMOTE_MISSING
                localTask.updatedAt == remoteTask.updatedAt -> {
                    return CustomResult.Success(ConflictResolution.NoConflict("Versions are identical"))
                }
                localTask.updatedAt.isAfter(remoteTask.updatedAt) -> ConflictType.LOCAL_NEWER
                else -> ConflictType.REMOTE_NEWER
            }
            
            // 3. 해결 전략 적용
            val resolution = when (strategy) {
                ConflictResolutionStrategy.SERVER_WINS -> resolveServerWins(localTask, remoteTask, conflictType)
                ConflictResolutionStrategy.CLIENT_WINS -> resolveClientWins(localTask, remoteTask, conflictType)
                ConflictResolutionStrategy.LATEST_TIMESTAMP -> resolveLatestTimestamp(localTask, remoteTask, conflictType)
    ...
    private suspend fun resolveServerWins(
        ...
        return when (conflictType) {
            ...
            ConflictType.REMOTE_MISSING -> {
                // 서버 우선이므로 로컬 데이터 삭제
                localTask?.let { localRepository.deleteEntity(it.id.value) }
            ...
            ConflictType.LOCAL_NEWER, ConflictType.REMOTE_NEWER -> {
                ...
                remoteTask?.let { localRepository.saveEntity(it) }
