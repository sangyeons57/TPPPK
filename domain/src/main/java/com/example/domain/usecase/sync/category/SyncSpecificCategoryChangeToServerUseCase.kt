...
/**
 * 로컬에서 특정 카테고리 변경사항을 서버로 동기화하는 UseCase
 * 특정 Outbox 항목 하나를 즉시 서버에 동기화합니다.
 */
class SyncSpecificCategoryChangeToServerUseCase @Inject constructor(
    ...
    suspend operator fun invoke(outboxId: String): CustomResult<SyncResult, Exception> {
        return try {
            ...
            // 2. 카테고리 컬렉션인지 확인
            if (outboxEntry.collectionName != "categories") {
                return CustomResult.Failure(Exception("Invalid collection name for category sync: ${outboxEntry.collectionName}"))
            }
    ...
    private suspend fun handleCreate(outboxEntry: OutboxEntity): CustomResult<Unit, Exception> {
        // 로컬에서 카테고리 조회 후 서버에 생성
        val categoryResult = localRepository.getEntityById(outboxEntry.documentId)
        return when (categoryResult) {
            is CustomResult.Success -> {
                val category = categoryResult.data
                remoteRepository.create(category).let { result ->
                    when (result) {
            ...
            else -> CustomResult.Failure(Exception("Category not found locally for CREATE operation"))
        }
    }
    
    private suspend fun handleUpdate(outboxEntry: OutboxEntity): CustomResult<Unit, Exception> {
        // 로컬에서 카테고리 조회 후 서버에 업데이트
        val categoryResult = localRepository.getEntityById(outboxEntry.documentId)
        return when (categoryResult) {
            is CustomResult.Success -> {
                val category = categoryResult.data
                val updateData = mapOf(
                    "name" to category.name.value,
                    "color" to category.color?.value,
                    "position" to category.position?.value,
                    "projectId" to category.projectId?.value,
            ...
            is CustomResult.Failure -> CustomResult.Failure(categoryResult.error)
            else -> CustomResult.Failure(Exception("Category not found locally for UPDATE operation"))
        }
    }
