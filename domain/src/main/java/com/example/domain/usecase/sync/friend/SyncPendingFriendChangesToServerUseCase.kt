...
/**
 * 로컬에서 대기 중인 모든 친구 변경사항을 서버로 동기화하는 UseCase
 * Outbox 패턴을 사용하여 오프라인 상태에서 발생한 변경사항을 처리합니다.
 */
class SyncPendingFriendChangesToServerUseCase @Inject constructor(
    ...
    private suspend fun handleCreate(outboxEntry: OutboxEntity): CustomResult<Unit, Exception> {
        // 로컬에서 친구 조회 후 서버에 생성
        val friendResult = localRepository.getEntityById(outboxEntry.documentId)
        return when (friendResult) {
            is CustomResult.Success -> {
                val friend = friendResult.data
                remoteRepository.create(friend).let { result ->
                    when (result) {
            ...
            else -> CustomResult.Failure(Exception("Friend not found locally for CREATE operation"))
        }
    }
    
    private suspend fun handleUpdate(outboxEntry: OutboxEntity): CustomResult<Unit, Exception> {
        // 로컬에서 친구 조회 후 서버에 업데이트
        val friendResult = localRepository.getEntityById(outboxEntry.documentId)
        return when (friendResult) {
            is CustomResult.Success -> {
                val friend = friendResult.data
            ...
            is CustomResult.Failure -> CustomResult.Failure(friendResult.error)
            else -> CustomResult.Failure(Exception("Friend not found locally for UPDATE operation"))
        }
    }
