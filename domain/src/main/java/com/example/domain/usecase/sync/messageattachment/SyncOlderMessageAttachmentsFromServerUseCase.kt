...
/**
 * 서버에서 오래된 메시지 첨부파일 데이터를 동기화하는 UseCase
 * SyncMetadata cursor를 기반으로 과거 데이터부터 가져옵니다.
 */
class SyncOlderMessageAttachmentsFromServerUseCase @Inject constructor(
    ...
    suspend operator fun invoke(limit: Int = 50): CustomResult<SyncResult, Exception> {
        return try {
            ...
            val updatedMetadata = syncMetadata.copy(
                lastSyncTimestamp = attachments.minByOrNull { it.updatedAt }?.updatedAt ?: Instant.now(),
