...
/**
 * 로컬에서 대기 중인 모든 DM 채널 변경사항을 서버로 동기화하는 UseCase
 * Outbox 패턴을 사용하여 오프라인 상태에서 발생한 변경사항을 처리합니다.
 */
class SyncPendingDMChannelChangesToServerUseCase @Inject constructor(
    ...
    suspend operator fun invoke(): CustomResult<SyncResult, Exception> {
        return try {
            // 1. DM 채널 관련 대기 중인 Outbox 항목들 조회
            val collectionName = "dmChannels"
    ...
    private suspend fun handleCreate(outboxEntry: OutboxEntity): CustomResult<Unit, Exception> {
        // 로컬에서 DM 채널 조회 후 서버에 생성
        val dmChannelResult = localRepository.getEntityById(outboxEntry.documentId)
        return when (dmChannelResult) {
            is CustomResult.Success -> {
                val dmChannel = dmChannelResult.data
                remoteRepository.create(dmChannel).let { result ->
                    when (result) {
            ...
            is CustomResult.Failure -> CustomResult.Failure(dmChannelResult.error)
            else -> CustomResult.Failure(Exception("DMChannel not found locally for CREATE operation"))
        }
    }
    
    private suspend fun handleUpdate(outboxEntry: OutboxEntity): CustomResult<Unit, Exception> {
        // 로컬에서 DM 채널 조회 후 서버에 업데이트
        val dmChannelResult = localRepository.getEntityById(outboxEntry.documentId)
        return when (dmChannelResult) {
            is CustomResult.Success -> {
                val dmChannel = dmChannelResult.data
                val updateData = mapOf(
                    "isBlocked" to dmChannel.isBlocked,
                    "lastMessageAt" to dmChannel.lastMessageAt,
                    "participant1Id" to dmChannel.participant1Id.value,
                    "participant2Id" to dmChannel.participant2Id.value,
                    "updatedAt" to dmChannel.updatedAt
                )
            ...
            is CustomResult.Failure -> CustomResult.Failure(dmChannelResult.error)
            else -> CustomResult.Failure(Exception("DMChannel not found locally for UPDATE operation"))
        }
    }
