...
/**
 * 서버에서 오래된 태스크 데이터를 동기화하는 UseCase
 * SyncMetadata cursor를 기반으로 과거 데이터부터 가져옵니다.
 */
class SyncOlderTasksFromServerUseCase @Inject constructor(
    ...
    suspend operator fun invoke(limit: Int = 50): CustomResult<SyncResult, Exception> {
        return try {
            ...
            tasks.forEach { task ->
                when (val saveResult = localRepository.saveEntity(task)) {
            ...
            val updatedMetadata = syncMetadata.copy(
                lastSyncTimestamp = tasks.minByOrNull { it.updatedAt }?.updatedAt ?: Instant.now(),
                lastServerCursor = tasks.firstOrNull()?.id?.value
            )
