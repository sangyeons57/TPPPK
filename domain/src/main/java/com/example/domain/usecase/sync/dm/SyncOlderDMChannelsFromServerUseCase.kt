...
/**
 * 서버에서 오래된 DM 채널 데이터를 동기화하는 UseCase
 * SyncMetadata cursor를 기반으로 과거 데이터부터 가져옵니다.
 */
class SyncOlderDMChannelsFromServerUseCase @Inject constructor(
    ...
    suspend operator fun invoke(limit: Int = 50): CustomResult<SyncResult, Exception> {
        return try {
            ...
            val dmChannels = when (serverResult) {
            ...
            dmChannels.forEach { dmChannel ->
                when (val saveResult = localRepository.saveEntity(dmChannel)) {
            ...
            val updatedMetadata = syncMetadata.copy(
                lastSyncTimestamp = dmChannels.minByOrNull { it.updatedAt }?.updatedAt ?: Instant.now(),
                lastServerCursor = dmChannels.firstOrNull()?.id?.value
            )
