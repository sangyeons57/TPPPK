...
/**
 * 로컬에서 변경된 DM 채널 데이터를 서버로 동기화하는 UseCase
 * SyncMetadata를 사용하여 마지막 동기화 이후 변경된 로컬 데이터만 처리합니다.
 */
class SyncPendingLocalDMChannelChangesToServerUseCase @Inject constructor(
        ...
        return try {
            ...
            dmChannels.forEach { dmChannel ->
                try {
                    ...
                    val remoteResult = remoteRepository.findById(dmChannel.id)
                    val syncResult = when (remoteResult) {
                        is CustomResult.Success -> {
                            ...
                            remoteRepository.update(dmChannel.id, updateData)
                        }
                        is CustomResult.Failure -> {
                            // 존재하지 않으면 생성
                            remoteRepository.create(dmChannel)
                        }
            ...
            val latestUpdateTime = dmChannels.maxByOrNull { it.updatedAt }?.updatedAt ?: Instant.now()
            val updatedMetadata = syncMetadata.copy(
                lastSyncTimestamp = latestUpdateTime,
                lastLocalCursor = dmChannels.firstOrNull()?.id?.value
            )
            syncMetadataDao.upsertSyncMetadata(updatedMetadata)
            
            CustomResult.Success(
                SyncResult(
                    totalCount = dmChannels.size,
