...
/**
 * DM 래퍼 데이터의 서버-로컬 간 충돌을 해결하는 UseCase
 * 충돌 감지 및 해결 전략을 적용합니다.
 */
class ResolveDMWrapperDataConflictUseCase @Inject constructor(
    private val remoteRepository: DefaultRepository<DMWrapper>,
    private val localRepository: LocalDMWrapperRepository
) {
    
    suspend operator fun invoke(
        dmWrapperId: DocumentId,
        strategy: ConflictResolutionStrategy = ConflictResolutionStrategy.SERVER_WINS
    ): CustomResult<ConflictResolution, Exception> {
        return try {
            // 1. 로컬과 서버에서 데이터 조회
            val localResult = localRepository.getEntityById(dmWrapperId.value)
            val localDMWrapper = when (localResult) {
                is CustomResult.Success -> localResult.data
                is CustomResult.Failure -> null
                else -> null
            }
            
            remoteRepository.setCollection(CollectionPath.from("dmWrappers"))
            val remoteResult = remoteRepository.findById(dmWrapperId)
            val remoteDMWrapper = when (remoteResult) {
            ...
            val conflictType = when {
                localDMWrapper == null && remoteDMWrapper == null -> {
                    return CustomResult.Success(ConflictResolution.NoConflict("Both versions are null"))
                }
                localDMWrapper == null -> ConflictType.LOCAL_MISSING
                remoteDMWrapper == null -> ConflictType.REMOTE_MISSING
                localDMWrapper.updatedAt == remoteDMWrapper.updatedAt -> {
                    return CustomResult.Success(ConflictResolution.NoConflict("Versions are identical"))
                }
                localDMWrapper.updatedAt.isAfter(remoteDMWrapper.updatedAt) -> ConflictType.LOCAL_NEWER
                else -> ConflictType.REMOTE_NEWER
            }
            
            // 3. 해결 전략 적용
            val resolution = when (strategy) {
                ConflictResolutionStrategy.SERVER_WINS -> resolveServerWins(localDMWrapper, remoteDMWrapper, conflictType)
                ConflictResolutionStrategy.CLIENT_WINS -> resolveClientWins(localDMWrapper, remoteDMWrapper, conflictType)
                ConflictResolutionStrategy.LATEST_TIMESTAMP -> resolveLatestTimestamp(localDMWrapper, remoteDMWrapper, conflictType)
                ConflictResolutionStrategy.MANUAL -> ConflictResolution.RequiresManualResolution(
                    conflictType = conflictType,
                    localData = localDMWrapper,
                    remoteData = remoteDMWrapper
                )
            }
    ...
    private suspend fun resolveServerWins(
        localDMWrapper: DMWrapper?,
        remoteDMWrapper: DMWrapper?,
        conflictType: ConflictType
    ): ConflictResolution {
        return when (conflictType) {
            ...
            ConflictType.REMOTE_MISSING -> {
                // 서버 우선이므로 로컬 데이터 삭제
                localDMWrapper?.let { localRepository.deleteEntity(it.id.value) }
            ...
            ConflictType.LOCAL_NEWER, ConflictType.REMOTE_NEWER -> {
                ...
                remoteDMWrapper?.let { localRepository.saveEntity(it) }
    ...
    sealed class ConflictResolution {
        ...
        data class RequiresManualResolution(
            ...
            val localData: DMWrapper?,
            val remoteData: DMWrapper?
        ) : ConflictResolution()
    }
}