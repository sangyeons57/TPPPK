...
/**
 * 서버에서 증분 프로젝트 채널 데이터를 동기화하는 UseCase
 * 마지막 동기화 시간 이후 변경된 데이터만 가져옵니다.
 */
class SyncIncrementalProjectChannelsFromServerUseCase @Inject constructor(
    ...
    suspend operator fun invoke(): CustomResult<SyncResult, Exception> {
        return try {
            ...
            val latestUpdateTime = channels.maxByOrNull { it.updatedAt }?.updatedAt ?: Instant.now()
