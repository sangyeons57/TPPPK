...
/**
 * 서버에서 오래된 프로젝트 데이터를 동기화하는 UseCase
 * SyncMetadata cursor를 기반으로 과거 데이터부터 가져옵니다.
 */
class SyncOlderProjectsFromServerUseCase @Inject constructor(
    ...
    suspend operator fun invoke(limit: Int = 50): CustomResult<SyncResult, Exception> {
        return try {
            ...
            val projects = when (serverResult) {
            ...
            projects.forEach { project ->
                when (val saveResult = localRepository.saveEntity(project)) {
            ...
            val updatedMetadata = syncMetadata.copy(
                lastSyncTimestamp = projects.minByOrNull { it.updatedAt }?.updatedAt ?: Instant.now(),
                lastServerCursor = projects.firstOrNull()?.id?.value
            )
