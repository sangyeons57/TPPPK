...
/**
 * 일정 데이터의 서버-로컬 간 충돌을 해결하는 UseCase
 * 충돌 감지 및 해결 전략을 적용합니다.
 */
class ResolveScheduleDataConflictUseCase @Inject constructor(
    ...
    suspend operator fun invoke(
        ...
        return try {
            // 1. 로컬과 서버에서 데이터 조회
            val localResult = localRepository.getEntityById(scheduleId.value)
            val localSchedule = when (localResult) {
                is CustomResult.Success -> localResult.data
                is CustomResult.Failure -> null
                else -> null
            }
            
            remoteRepository.setCollection(CollectionPath.from("schedules"))
            val remoteResult = remoteRepository.findById(scheduleId)
            val remoteSchedule = when (remoteResult) {
            ...
            val conflictType = when {
                localSchedule == null && remoteSchedule == null -> {
                    return CustomResult.Success(ConflictResolution.NoConflict("Both versions are null"))
                }
                localSchedule == null -> ConflictType.LOCAL_MISSING
                remoteSchedule == null -> ConflictType.REMOTE_MISSING
                localSchedule.updatedAt == remoteSchedule.updatedAt -> {
                    return CustomResult.Success(ConflictResolution.NoConflict("Versions are identical"))
                }
                localSchedule.updatedAt.isAfter(remoteSchedule.updatedAt) -> ConflictType.LOCAL_NEWER
                else -> ConflictType.REMOTE_NEWER
            }
            
            // 3. 해결 전략 적용
            val resolution = when (strategy) {
                ConflictResolutionStrategy.SERVER_WINS -> resolveServerWins(localSchedule, remoteSchedule, conflictType)
                ConflictResolutionStrategy.CLIENT_WINS -> resolveClientWins(localSchedule, remoteSchedule, conflictType)
                ConflictResolutionStrategy.LATEST_TIMESTAMP -> resolveLatestTimestamp(localSchedule, remoteSchedule, conflictType)
                ConflictResolutionStrategy.MANUAL -> ConflictResolution.RequiresManualResolution(
                    conflictType = conflictType,
                    localData = localSchedule,
                    remoteData = remoteSchedule
                )
            }
    ...
    private suspend fun resolveServerWins(
        ...
        return when (conflictType) {
            ...
            ConflictType.REMOTE_MISSING -> {
                // 서버 우선이므로 로컬 데이터 삭제
                localSchedule?.let { localRepository.deleteEntity(it.id.value) }
            ...
            ConflictType.LOCAL_NEWER, ConflictType.REMOTE_NEWER -> {
                ...
                remoteSchedule?.let { localRepository.saveEntity(it) }
    ...
    sealed class ConflictResolution {
        ...
        data class RequiresManualResolution(
            ...
            val localData: Schedule?,
            val remoteData: Schedule?
        ) : ConflictResolution()
    }
}