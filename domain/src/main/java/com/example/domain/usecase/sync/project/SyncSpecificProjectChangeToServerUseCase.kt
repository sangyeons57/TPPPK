...
/**
 * 로컬에서 특정 프로젝트 변경사항을 서버로 동기화하는 UseCase
 * 특정 Outbox 항목 하나를 즉시 서버에 동기화합니다.
 */
class SyncSpecificProjectChangeToServerUseCase @Inject constructor(
    ...
    suspend operator fun invoke(outboxId: String): CustomResult<SyncResult, Exception> {
        return try {
            ...
            // 2. 프로젝트 컬렉션인지 확인
            if (outboxEntry.collectionName != "projects") {
                return CustomResult.Failure(Exception("Invalid collection name for project sync: ${outboxEntry.collectionName}"))
            }
    ...
    private suspend fun handleCreate(outboxEntry: OutboxEntity): CustomResult<Unit, Exception> {
        // 로컬에서 프로젝트 조회 후 서버에 생성
        val projectResult = localRepository.getEntityById(outboxEntry.documentId)
        return when (projectResult) {
            is CustomResult.Success -> {
                val project = projectResult.data
                remoteRepository.create(project).let { result ->
                    when (result) {
            ...
            is CustomResult.Failure -> CustomResult.Failure(projectResult.error)
            else -> CustomResult.Failure(Exception("Project not found locally for CREATE operation"))
        }
    }
    
    private suspend fun handleUpdate(outboxEntry: OutboxEntity): CustomResult<Unit, Exception> {
        // 로컬에서 프로젝트 조회 후 서버에 업데이트
        val projectResult = localRepository.getEntityById(outboxEntry.documentId)
        return when (projectResult) {
            is CustomResult.Success -> {
                val project = projectResult.data
                val updateData = mapOf(
                    ...
                    "description" to project.description?.value,
                    "profileImageUrl" to project.profileImageUrl?.value,
                    "inviteCode" to project.inviteCode?.value,
                    "ownerId" to project.ownerId.value,
            ...
            is CustomResult.Failure -> CustomResult.Failure(projectResult.error)
            else -> CustomResult.Failure(Exception("Project not found locally for UPDATE operation"))
        }
    }
