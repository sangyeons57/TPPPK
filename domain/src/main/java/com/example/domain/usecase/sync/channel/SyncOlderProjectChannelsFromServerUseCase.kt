...
/**
 * 서버에서 오래된 프로젝트 채널 데이터를 동기화하는 UseCase
 * SyncMetadata cursor를 기반으로 과거 데이터부터 가져옵니다.
 */
class SyncOlderProjectChannelsFromServerUseCase @Inject constructor(
    ...
    suspend operator fun invoke(limit: Int = 50): CustomResult<SyncResult, Exception> {
        return try {
            ...
            val channels = when (serverResult) {
            ...
            channels.forEach { channel ->
                when (val saveResult = localRepository.saveEntity(channel)) {
            ...
            val updatedMetadata = syncMetadata.copy(
                lastSyncTimestamp = channels.minByOrNull { it.updatedAt }?.updatedAt ?: Instant.now(),
                lastServerCursor = channels.firstOrNull()?.id?.value
            )
