...
/**
 * 서버에서 오래된 일정 데이터를 동기화하는 UseCase
 * SyncMetadata cursor를 기반으로 과거 데이터부터 가져옵니다.
 */
class SyncOlderSchedulesFromServerUseCase @Inject constructor(
    ...
    suspend operator fun invoke(limit: Int = 50): CustomResult<SyncResult, Exception> {
        return try {
            ...
            val schedules = when (serverResult) {
            ...
            schedules.forEach { schedule ->
                when (val saveResult = localRepository.saveEntity(schedule)) {
            ...
            val updatedMetadata = syncMetadata.copy(
                lastSyncTimestamp = schedules.minByOrNull { it.updatedAt }?.updatedAt ?: Instant.now(),
                lastServerCursor = schedules.firstOrNull()?.id?.value
            )
