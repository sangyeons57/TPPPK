...
/**
 * 서버에서 오래된 DM 래퍼 데이터를 동기화하는 UseCase
 * SyncMetadata cursor를 기반으로 과거 데이터부터 가져옵니다.
 */
class SyncOlderDMWrappersFromServerUseCase @Inject constructor(
    ...
    suspend operator fun invoke(limit: Int = 50): CustomResult<SyncResult, Exception> {
        return try {
            ...
            val dmWrappers = when (serverResult) {
            ...
            dmWrappers.forEach { dmWrapper ->
                when (val saveResult = localRepository.saveEntity(dmWrapper)) {
            ...
            val updatedMetadata = syncMetadata.copy(
                lastSyncTimestamp = dmWrappers.minByOrNull { it.updatedAt }?.updatedAt ?: Instant.now(),
                lastServerCursor = dmWrappers.firstOrNull()?.id?.value
            )
