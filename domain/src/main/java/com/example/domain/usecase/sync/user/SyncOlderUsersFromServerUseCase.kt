...
/**
 * 서버에서 오래된 사용자 데이터를 동기화하는 UseCase
 * SyncMetadata cursor를 기반으로 과거 데이터부터 가져옵니다.
 */
class SyncOlderUsersFromServerUseCase @Inject constructor(
    ...
    suspend operator fun invoke(limit: Int = 50): CustomResult<SyncResult, Exception> {
        return try {
            ...
            val updatedMetadata = syncMetadata.copy(
                lastSyncTimestamp = users.minByOrNull { it.updatedAt }?.updatedAt ?: Instant.now(),
                lastServerCursor = users.firstOrNull()?.id?.value
            )
