...
interface FetchPastMessagesLocalUseCase {
    suspend operator fun invoke(
        channelId: DocumentId,
        beforeMessageId: DocumentId?,
        limit: Int = 20
    ): CustomResult<List<Message>, Exception>
}

class FetchPastMessagesLocalUseCaseImpl @Inject constructor(
    private val messageLocalRepository: MessageLocalRepository
) : FetchPastMessagesLocalUseCase {
    ...
    override suspend operator fun invoke(
        ...
        return TODO("로컬 저장소에서 이전 메시지들을 페이지네이션으로 조회")
    }
} 