...
/**
 * 로컬에서 대기 중인 모든 메시지 첨부파일 변경사항을 서버로 동기화하는 UseCase
 * Outbox 패턴을 사용하여 오프라인 상태에서 발생한 변경사항을 처리합니다.
 */
class SyncPendingMessageAttachmentChangesToServerUseCase @Inject constructor(
    ...
    suspend operator fun invoke(): CustomResult<SyncResult, Exception> {
        return try {
            // 1. 메시지 첨부파일 관련 대기 중인 Outbox 항목들 조회
    ...
    private suspend fun handleCreate(outboxEntry: OutboxEntity): CustomResult<Unit, Exception> {
        // 로컬에서 메시지 첨부파일 조회 후 서버에 생성
        val attachmentResult = localRepository.getEntityById(outboxEntry.documentId)
        return when (attachmentResult) {
            is CustomResult.Success -> {
                val attachment = attachmentResult.data
                remoteRepository.create(attachment).let { result ->
                    when (result) {
            ...
            is CustomResult.Failure -> CustomResult.Failure(attachmentResult.error)
            else -> CustomResult.Failure(Exception("MessageAttachment not found locally for CREATE operation"))
        }
    }
    
    private suspend fun handleUpdate(outboxEntry: OutboxEntity): CustomResult<Unit, Exception> {
        // 로컬에서 메시지 첨부파일 조회 후 서버에 업데이트
        val attachmentResult = localRepository.getEntityById(outboxEntry.documentId)
        return when (attachmentResult) {
            is CustomResult.Success -> {
                val attachment = attachmentResult.data
            ...
            is CustomResult.Failure -> CustomResult.Failure(attachmentResult.error)
            else -> CustomResult.Failure(Exception("MessageAttachment not found locally for UPDATE operation"))
        }
    }
