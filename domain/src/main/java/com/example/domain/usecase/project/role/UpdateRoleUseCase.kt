...
interface UpdateRoleUseCase {
    suspend operator fun invoke(projectId: String, roleId: String, newName: String, newPermissions: Map<RolePermission, Boolean>, isDefault: Boolean? = null): Result<Unit>
}

class UpdateRoleUseCaseImpl @Inject constructor(
    private val projectRoleRepository: ProjectRoleRepository
) : UpdateRoleUseCase {
    override suspend operator fun invoke(projectId: String, roleId: String, newName: String, newPermissions: Map<RolePermission, Boolean>, isDefault: Boolean?): Result<Unit> {
        if (newName.isBlank()) {
