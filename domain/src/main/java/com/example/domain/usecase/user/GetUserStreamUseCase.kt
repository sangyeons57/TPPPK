...
interface GetUserStreamUseCase {
    suspend operator fun invoke(userId: DocumentId): Flow<CustomResult<User, Exception>>
}

class GetUserStreamUseCaseImpl @Inject constructor(
    private val userRepository: UserRepository
) : GetUserStreamUseCase {
    override suspend operator fun invoke(userId: DocumentId): Flow<CustomResult<User, Exception>> {
        return when (val result = userRepository.observe(userId).first()) {
