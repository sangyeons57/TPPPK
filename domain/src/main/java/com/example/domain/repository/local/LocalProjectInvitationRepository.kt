 ...
 * - ProjectInvitation 도메인 특화 기능만 추가 정의 (20%)
 * - Flow로 UI에 실시간 데이터 제공 (Observer Pattern)
 * - Outbox 관리 (동기화 대상 저장)
 *
 * 📋 BaseLocalRepository 상속 메서드:
 * - observeEntityById -> observeInvitationById
 * - observeAllEntities -> observeAllInvitations
 * - observeEntityUpdatedAt -> observeInvitationUpdatedAt
 * - getEntityById -> getInvitationById
 * - getEntitiesByIds -> getInvitationsByIds
 * - getAllEntities -> getAllInvitations
 * - saveEntity -> saveInvitation
 * - saveEntities -> saveInvitations
 * - deleteEntity -> deleteInvitation
 * - Plus SyncableRepository methods (addToOutbox, clearAllEntities, etc.)
 */
interface LocalProjectInvitationRepository : BaseLocalRepository<ProjectInvitation> {

    // === BaseLocalRepository 메서드 (구현체에서 초대 전용 메서드로 매핑) ===
    // observeEntityById -> observeInvitationById
    // observeAllEntities -> observeAllInvitations  
    // observeEntityUpdatedAt -> observeInvitationUpdatedAt
    // getEntityById -> getInvitationById
    ...
    // saveEntities -> saveInvitations
    // deleteEntity -> deleteInvitation
    // getEntitiesUpdatedAfter -> getInvitationsUpdatedAfter
    // clearAllEntities -> clearAllInvitations
    // getTotalEntityCount -> getTotalInvitationCount
    // entityExists -> invitationExists
